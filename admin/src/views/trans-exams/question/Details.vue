...
import { TRANEXAM_QUESTION_DETAILS, PAGE_NOT_FOUND, EDIT_TRANEXAM_QUESTION, UPDATE_TRANEXAM_QUESTION, 
...
const name = 'TransExamQuestionDetailsView'
...
const question = computed(() => store.state.trans_exam_questions.question)
...
function fetchData(id) {
	...
	store.dispatch(TRANEXAM_QUESTION_DETAILS, id)
	.catch(error => {
...
function edit() {
	store.commit(CLEAR_ERRORS)
	store.dispatch(EDIT_TRANEXAM_QUESTION, question.value.id)
	.then(model => {
		state.form.model = deepClone(model)
		state.form.type = ENTITY_TYPES.QUESTION.name
		state.form.action = UPDATE_TRANEXAM_QUESTION
		state.form.title = `編輯題目`
...
<template>
	<div v-if="!isEmptyObject(question)">
		<v-card>
			<CommonCardTitle :id="question.id" title="題目"
			:tooltip="`編輯題目資料`" :can_cancel="false"
			@edit="edit" 
			/>
			<v-card-text>
				<TransExamQuestionView :model="question" />
			</v-card-text>
		</v-card>
